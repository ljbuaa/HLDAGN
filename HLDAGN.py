"""
@author: luo jie
code for HLDAGN in the paper
"Visual Image Decoding of Brain Activities using a Hierarchical Latent Dual-Attentional Generative Network"
"""
...
class MKCBlock(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        out_x1 = x1 * att_a.expand_as(x1)
        out_x2 = x2 * att_b.expand_as(x2)
        out_x3 = x3 * att_c.expand_as(x3)
...
class MultiScaleFusion(nn.Module):
    def __init__(self, out_size=64):
        ...
        self.avgMap8 = nn.Conv2d(32, 32, kernel_size=1, bias=False)
        self.avgMap16 = nn.Conv2d(32, 32, kernel_size=1, bias=False)
        self.avgMap32 = nn.Conv2d(32, 32, kernel_size=1, bias=False)
        self.avgMap64 = nn.Conv2d(32, 32, kernel_size=1, bias=False)
    ...
    def forward(self, fea8,fea16,fea32,fea64):
        ...
        out_avgMap8 = self.avgMap8(out8).unsqueeze(1)
        out_avgMap16 = self.avgMap16(out16).unsqueeze(1)
        out_avgMap32 = self.avgMap32(out32).unsqueeze(1)
        out_avgMap64 = self.avgMap64(fea64).unsqueeze(1)
        out = torch.cat([out_avgMap8, out_avgMap16, out_avgMap32, out_avgMap64], dim=1)
        out = F.sigmoid(out)
        att8 = out[:,0]
        att16 = out[:,1]
        att32 = out[:,2]
